 ...
 *** This is the client program for Diffie-Hellman Key Exchange Protocol   ***
...
void Talk_to_server ( int cfd )
{
  ...
  /* send the public key to server */
   ...
   strcpy(send_msg.buf,"Public Key from Alice (User A)\n");
                
   /* send the public key to Bob */
   send_msg.dummy1 = q;  /* q */
   send_msg.dummy2 = alpha; /* alpha */
   send_msg.dummy3 = Y_A;  /* value of public key */
   ...
   case PUBKEY : /* Public key */
               ...
               printf("Received public values from Bob are q = %ld, alpha = %ld, Y_B = %ld\n\r", 
                       recv_msg.dummy1, recv_msg.dummy2, Y_B);
               ...
               /* Send an acknowledgement to Bob that the secret key is computed */       
                ...
                send_msg.hdr.src_addr = src_addr;
                send_msg.hdr.dest_addr = dest_addr;
                strcpy(send_msg.buf,"Acknowledgement Message from Alice (User A): secret key computed...\n");
...
int main ( int argc, char *argv[] )
{
   ...
   printf("******* This is client side demo for implementing Diffie-Hellman Key Exchange Protocol *****\n\n");
