...
int main(int argc, char **argv)
{
     ...
     /* Modified by tmaheshb
      * set structure member to zero in main
      * so that source port can be modified by -S option
      */

     memset(&data.sa_local, 0, sizeof(data.sa_local));
     data.sa_local.sin_family = AF_INET;
     //data.sa_local.sin_port = htons(INPORT_ANY);
     //data.sa_local.sin_addr = inet_addr(INADDR_ANY);
...
int tftp_cmd_line_options(int argc, char **argv)
{
     ...
     while ((c = getopt_long(argc, argv, /*"gpl:r:Vh"*/ "gpl:r:Vht:b:smP:S:",
                             options, &option_index)) != EOF)
     {
	  ...
	  case 'S' :
                interactive = 0;
		data.sa_local.sin_port = htons(atoi(optarg));
		break;
#if 1
          case 't':
...
void tftp_usage(void)
{
     ...
     fprintf(stderr,
             ...
             "  -S --src-port <portno>   : tftp client source port\n"
#if 0
